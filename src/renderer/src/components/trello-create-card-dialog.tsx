...
export function TrelloCreateCardDialog({
  ...
  const loadListsForBoard = async (nextBoardId: string): Promise<void> => {
    if (!nextBoardId) {
      setLists([])
      setListId('')
      return
  ...
  useEffect(() => {
    ...
    void loadListsForBoard(nextBoardId)
    // Why: this effect seeds the modal from freshly-opened props; user board changes
    // are handled directly by the Board popover to avoid duplicate list reloads.
  ...
  const handleCreate = async (): Promise<void> => {
    ...
    try {
      ...
      const card = await fetchTrelloCard(result.id)
      if (card) {
        onCreated(card)
      }
      reset()
                  ...
                  {boards.length === 0 ? (
                    ...
                    boards.map((board) => (
                      <button
                        ...
                        onClick={() => {
                          setBoardId(board.id)
                          void loadListsForBoard(board.id)
                        }}
